"""Updated version of the Gradio demo."""
import re
...
def display_result_for_mini_imagenet_sample(
    ...
    if isinstance(label, str):
        ...
        outputs = ensemble_gradcam.apply(image, label[0])
    else:
        raise ValueError("label must be str or list. Label is {}".format(label))
    ...
    pred_dist = [outputs[m + "_pred_dist"] for m in modes]
    pred_label = [outputs[m + "_label"] for m in modes]

    path = path.format(y_true, suffix)
    path = join(REPO_PATH, "experiments", "results", "plots", path)

    # class name to predicted probablity mapping
    if isinstance(label, str):
        true_dist = [1.0 if class_name_map(i) == label else 0.0 for i in range(len(classes))]
    elif isinstance(label, list):
        true_dist = [1.0 / len(label) if class_name_map(i) in label else 0.0 for i in range(len(classes))]
    else:
        raise ValueError("label must be int or list")

    class_to_prob = [{class_name_map(i): true_dist[i] for i in range(len(classes))}]
    class_to_prob += [
        {class_name_map(i): np.round(pred_dist[x][i].item(), decimals=3) for i in range(len(classes))}
        for x in range(len(pred_dist))
    ]
...
def display_result_for_counterfactual_sample(cgn, ys, classes, clf_transforms, ensemble_gradcam):
    ...
    result = display_result_for_mini_imagenet_sample(
